            ...
            for (int y = 0; y < volDim[1] - 1; y++) {
                for (int x = 0; x < volDim[0] - 1; x++) {
                    ...
                    while (TablesMC.MC_TRI_TABLE[cubeindex + i] != -1) {
                        ...
                        // Add triangles vertices normalized with the maximal possible value
                        Vector3f a = new Vector3f(vertList[index3][0] / maxAxisVal - 0.5f, vertList[index3][1] / maxAxisVal - 0.5f, vertList[index3][2] / maxAxisVal - 0.5f);
                        Vector3f b = new Vector3f(vertList[index2][0] / maxAxisVal - 0.5f, vertList[index2][1] / maxAxisVal - 0.5f, vertList[index2][2] / maxAxisVal - 0.5f);
                        Vector3f c = new Vector3f(vertList[index1][0] / maxAxisVal - 0.5f, vertList[index1][1] / maxAxisVal - 0.5f, vertList[index1][2] / maxAxisVal - 0.5f);

                        vertices.add(a.getArray());
                        vertices.add(b.getArray());
                        vertices.add(c.getArray());

                        Vector3f ab = Vector3f.sub(a, b, null);
                        Vector3f ac = Vector3f.sub(a, c, null);

                        Vector3f result = Vector3f.cross(ab, ac, null);
                        normals.add(result.getArray());
                        normals.add(result.getArray());
                        normals.add(result.getArray());
                        i += 3;
                    }
