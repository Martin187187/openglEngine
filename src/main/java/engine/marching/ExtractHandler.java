...
public class ExtractHandler {
	public static void extractHandlerChar(File inputFile, File outFile, final int[] size, final float voxSize[],
			final char isoValue, int nThreads) {
		char[] scalarField;
		...
		for (int i = 0; i < nThreads; i++) {
			...
			Thread t = new Thread() {
				public void run() {
					MarchingCubes.marchingCubesChar(finalScalarField, new int[] { size[0], size[1], paddedSegmentSize },
	...
	public static void extractHandlerShort(File inputFile, File outFile, final int[] size, final float voxSize[],
			final short isoValue, int nThreads) {
		short[] scalarField;
		...
		for (int i = 0; i < nThreads; i++) {
			...
			Thread t = new Thread() {
				public void run() {
					MarchingCubes.marchingCubesShort(finalScalarField,
							new int[] { size[0], size[1], paddedSegmentSize }, size[2], voxSize, isoValue,
							finalZAxisOffset, callback);
				}
			};
	...
	public static Mesh extractHandlerInt(File inputFile, File outFile, final int[] size, final float voxSize[],
...
//		outputToFile(vertecies, normals, outFile);
	...
	public static void extractHandlerFloat(File inputFile, File outFile, final int[] size, final float voxSize[],
		...
		for (int i = 0; i < nThreads; i++) {
			...
			Thread t = new Thread() {
				public void run() {
					MarchingCubes.marchingCubesFloat(finalScalarField,
							new int[] { size[0], size[1], paddedSegmentSize }, size[2], voxSize, isoValue,
							finalZAxisOffset, callback);
				}
			};
	...
	public static void extractHandlerDouble(File inputFile, File outFile, final int[] size, final float voxSize[],
			final double isoValue, int nThreads) {
		double[] scalarField;
		...
		for (int i = 0; i < nThreads; i++) {
			...
			Thread t = new Thread() {
				public void run() {
					MarchingCubes.marchingCubesDouble(finalScalarField,
							new int[] { size[0], size[1], paddedSegmentSize }, size[2], voxSize, isoValue,
							finalZAxisOffset, callback);
				}
			};
	...
	private static void outputToFile(ArrayList<ArrayList<float[]>> results, ArrayList<ArrayList<float[]>> normalList, File outFile) {
		try {
			BufferedOutputStream stream = new BufferedOutputStream(new FileOutputStream(outFile));
			...
			for (int i = 0; i < results.size(); i++) {
				ArrayList<float[]> resSeg = results.get(i);
				ArrayList<float[]> resSegNorm = normalList.get(i);

				for (int j = 0; j < resSeg.size(); j++) {
					if (idx % 3 == 0) {
						stream.write(("f " + (idx + 1) + " " + (idx + 2) + " " + (idx + 3) + "\n").getBytes());
						stream.write(("n " + resSegNorm.get(j)[0] + " " + resSegNorm.get(j)[1] + " " + resSegNorm.get(j)[2] + "\n").getBytes());
					}
					idx++;
	...
	private static Mesh getMesh(ArrayList<ArrayList<float[]>> vertexList, ArrayList<ArrayList<float[]>> normalList) {
		...
		for (int i = 0; i < vertexList.size(); i++)
			length += vertexList.get(i).size();

		float[] vertecies = new float[length*3];
		float[] normals = new float[length];
		int[] indecies = new int[length];
		
		int idx = 0;
		for (int i = 0; i < vertexList.size(); i++) {
			ArrayList<float[]> resSeg = vertexList.get(i);
			ArrayList<float[]> resSegNorm = normalList.get(i);

			for (int j = 0; j < resSeg.size(); j++) {
				if (idx % 3 == 0) {
					indecies[idx + 0] = idx + 0;
					indecies[idx + 1] = idx + 1;
					indecies[idx + 2] = idx + 2;

					normals[i+j + 0] = resSegNorm.get(j)[0];
					normals[i+j + 1] = resSegNorm.get(j)[1];
					normals[i+j + 2] = resSegNorm.get(j)[2];
				}
				idx++;

				vertecies[i+j*3+0] = resSeg.get(j)[0];
				vertecies[i+j*3+1] = resSeg.get(j)[1];
				vertecies[i+j*3+2] = resSeg.get(j)[2];
			}
		}


		return new Mesh(vertecies, normals, indecies);
	}
}
